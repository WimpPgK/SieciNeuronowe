...
public class Siec 
{
	...
	public Siec()
	{
		...
		for (numerEpoki = 0 ; numerEpoki < 100 ; numerEpoki++)
		{
...
//***********************************************
	void dzialaj(int a)		// arg to numer iteracji
	{
		...
		*/
		...
		System.out.println("//*********************************");
		*/
		int sasiedztwo = obliczSasiedztwo();
		
		for(int i = 0 ; i < sasiedztwo ; i++)
		{	
			n[odleglosc[i].kolumna][odleglosc[i].wiersz].korektaWag(wsp);
		}
...
/*
	...
	// ze wzoru 3*(e^(-(numerEpoki/5))			 - 3 to liczba sasiadow od ktorej zaczynamy
	private int obliczSasiedztwo()
	{
		int poczatkowaSzerokoscKratownicy = 5;  //(liczba sasiadow)
		double e = 2.71828182845;
		double lambda = 5;
		double potega = -numerEpoki/lambda;
		
		
		return (int) (poczatkowaSzerokoscKratownicy*Math.pow(e, potega));
	}
}
